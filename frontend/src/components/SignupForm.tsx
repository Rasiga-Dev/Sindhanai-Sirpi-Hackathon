...
const SignupForm: React.FC = () => {
  ...
  const handleUDISEChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (code.length === 11) {
      try {
        const res = await axios.get(`https://sindhanai-sirpi-hackathon.onrender.com/api/schools/udise/${code}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!passwordError && !confirmError) {
      try {
        const res = await axios.post('https://sindhanai-sirpi-hackathon.onrender.com/api/schools/register', {
